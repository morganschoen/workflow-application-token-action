...
async function run() {
  ...
  if (app) {
    ...
    try {
      ...
      const repository = process.env['GITHUB_REPOSITORY_CUSTOM'];

      if (!repository || repository.trim().length === 0) {
        throw new Error(`The repository value was missing from the environment as 'GITHUB_REPOSITORY_CUSTOM'`);
      }
